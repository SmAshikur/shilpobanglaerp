    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-indigo-100/50 overflow-hidden">
        <div class="p-8 md:p-12">
            <div class="flex items-center gap-4 mb-10">
                ...
                <div>
                    <h3 class="text-2xl font-bold text-slate-800">Edit Professional</h3>
                    <p class="text-slate-500">Update the details of {{ $member->name }}</p>
                </div>
            </div>

            <form action="{{ route('dashboard.team.update', $member) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ $member->name }}" required placeholder="John Doe" 
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="position" value="{{ $member->position }}" required placeholder="e.g. Senior Developer" 
                               class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white focus:outline-none transition-all duration-300">
                    </div>
                </div>
                ...
                <div class="grid md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="url" name="facebook_url" value="{{ $member->facebook_url }}" placeholder="https://..." 
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="url" name="linkedin_url" value="{{ $member->linkedin_url }}" placeholder="https://..." 
                                   class="w-full pl-14 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white focus:outline-none transition-all duration-300">
                        </div>
                    </div>
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="url" name="twitter_url" value="{{ $member->twitter_url }}" placeholder="https://..." 
                                   class="w-full pl-14 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white focus:outline-none transition-all duration-300">
                        </div>
                    </div>
                </div>

                <div class="space-y-4" x-data="{ preview: '{{ $member->image ? asset('storage/'.$member->image) : null }}' }">
                    ...
                    <div class="flex items-center gap-8 bg-slate-50 p-8 rounded-3xl border border-slate-100">
                        ...
                        <div class="flex-1">
                            <h4 class="font-bold text-slate-800 text-sm mb-1">Update Photo</h4>
                            <p class="text-xs text-slate-500 mb-4">Leave empty to keep the current photo. Recommended: Square image</p>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="bio" rows="4" placeholder="Brief introduction about the member..." 
                              class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white focus:outline-none transition-all duration-300 resize-none">{{ $member->bio }}</textarea>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-8 border-t border-slate-100 pt-8 mt-8">
                    ...
                    <label class="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl cursor-pointer hover:bg-slate-100 transition">
                        ...
                        <div class="relative">
                            <input type="checkbox" name="is_featured" value="1" {{ $member->is_featured ? 'checked' : '' }} class="peer sr-only">
                            <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                        </div>
                    </label>
                </div>
