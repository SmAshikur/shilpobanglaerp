...
@section('content')
<div class="max-w-6xl mx-auto" x-data="invoiceForm({{ $invoice->items->toJson() }})">
    <form action="{{ route('dashboard.erp.invoices.update', $invoice) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="flex items-center justify-between mb-8">
            <div>
                <h2 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Edit Invoice</h2>
                <p class="text-slate-500 dark:text-slate-400 mt-1 font-medium italic">Modifying {{ $invoice->invoice_number }}</p>
            </div>
            <div class="flex gap-4">
                <button type="submit" name="status" value="draft" class="px-6 py-3 {{ $invoice->status === 'draft' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-400' }} font-bold rounded-2xl hover:opacity-90 transition-all">
                    Save as Draft
                </button>
                <button type="submit" name="status" value="unpaid" class="px-6 py-3 {{ $invoice->status !== 'draft' ? 'bg-indigo-600 text-white' : 'bg-emerald-600 text-white' }} font-bold rounded-2xl hover:opacity-90 shadow-xl transition-all">
                    {{ $invoice->status === 'draft' ? 'Publish & Send' : 'Update Invoice' }}
                </button>
            </div>
        </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-2xl shadow-slate-200/50 dark:shadow-none p-10">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" value="{{ $invoice->invoice_number }}" disabled class="w-full px-6 py-4 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border-none outline-none font-bold text-slate-400 cursor-not-allowed">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Issue Date</label>
                            <input type="date" name="invoice_date" value="{{ $invoice->invoice_date }}" required class="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-700 outline-none font-bold text-slate-700 dark:text-white transition-all">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Due Date</label>
                            <input type="date" name="due_date" value="{{ $invoice->due_date }}" class="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-700 outline-none font-bold text-slate-700 dark:text-white transition-all">
                        </div>
                    </div>
...
<script>
    function invoiceForm(existingItems = []) {
        let initialItems = existingItems.map(i => ({
            desc: i.description,
            qty: i.quantity,
            price: i.unit_price
        }));
        ...
        if (initialItems.length === 0) {
            initialItems = [{ desc: '', qty: 1, price: 0 }];
        }
