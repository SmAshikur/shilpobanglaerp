...
<div class="max-w-4xl">
    ...
    <form action="{{ route('dashboard.erp.companies.update', $company->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-none p-8 md:p-12">
            <div class="grid md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ $company->name }}" required placeholder="Enter company name" class="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-600 outline-none transition duration-500 font-medium dark:text-white">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="email" name="email" value="{{ $company->email }}" placeholder="email@company.com" class="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-600 outline-none transition duration-500 font-medium dark:text-white">
                </div>
